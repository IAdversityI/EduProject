App.getID('exitLogin').addEventListener('tap', function() {
		if(App.getItem("tokenflag")) {
			App.removeItem("tokenflag");
			App.removeItem("cache_1");
			App.removeItem("nickNameflag");
			App.evalJS("./pages/main_mine.html", "removeImageName()");
...
mui.plusReady(function() {
	plus.runtime.getProperty(plus.runtime.appid, function(b) {
		App.getID("Vname").innerHTML = b.version;
	});
})
...
App.getID("updateVer").addEventListener("tap", function() {
	update();
})